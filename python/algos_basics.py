...
def get_mic_delta(r0, r1, azimuth, elevation=None):
    ...
    if len(r0) == 3:
        ...
        s = np.array((np.cos(azimuth) * np.cos(elevation), np.sin(azimuth) * np.cos(elevation), np.sin(elevation)))
    # 2d source
    elif len(r0) == 2:
        assert len(r0) == 2
...
def get_autocorrelation(signals, frequency_bins=None):
    ...
    if signals.shape[0] > signals.shape[1]:
        print("Warning: probably shape mismatch for signals (expecting num_mics in frist dimension)", signals.shape)

    signals_f = np.fft.rfft(signals, n=signals.shape[1], axis=1).T  # num_samples x num_mics
    if frequency_bins is None:
        Rx = 1 / num_mics * signals_f[:, :, np.newaxis] @ signals_f[:, np.newaxis, :].conj()
    else:
        Rx = 1 / num_mics * signals_f[frequency_bins, :, np.newaxis] @ signals_f[frequency_bins, np.newaxis, :].conj()
    return Rx
...
def get_responses_DAS_old(Rx, mic_positions, omega=None, num_angles=1000):
    ...
    for az in azimuth_array:
        for el in el_array:
            h = [
                np.exp(-1j * omega * get_mic_delta(ref_mic, mic_pos, az, el) / SPEED_OF_SOUND)
...
def get_responses_DAS_old(Rx, mic_positions, omega=None, num_angles=1000):
    ...
    for az in azimuth_array:
        for elevation in elevation_array:
            h = [
                np.exp(-1j * omega * get_mic_delta(ref_mic, mic_pos, az, elevation) / SPEED_OF_SOUND)
                for mic_pos in mic_positions
            ]
...
if __name__ == "__main__":
    ...
    mic_number = 5
    dimension = 2
    mic_positions_2d = get_uniform_array(mic_number=mic_number, baseline=baseline, dimension=dimension)

    # testing delays for pi_2 and 0
    azimuth_array = np.linspace(0, 2 * np.pi, 360)
    deltas = np.array(
        [get_mic_delta(mic_positions_2d[0], mic_positions_2d[1], az) / SPEED_OF_SOUND for az in azimuth_array])
    assert np.allclose(deltas[azimuth_array == np.pi / 2.0], 0), deltas[azimuth_array == np.pi / 2.0]
    assert np.allclose(deltas[azimuth_array == -np.pi / 2.0], 0), deltas[azimuth_array == np.pi / 2.0]
    assert np.allclose(deltas[azimuth_array == 0], np.max(deltas)), deltas[azimuth_array == 0]
    assert np.allclose(deltas[azimuth_array == np.pi], np.min(deltas)), deltas[azimuth_array == np.pi]
    ...
    deltas = np.array(
        [get_mic_delta(mic_positions_2d[0], mic_positions_2d[1], az, el) / SPEED_OF_SOUND for el in elevation_array])
    assert np.allclose(deltas[elevation_array == np.pi / 2.0], 0), deltas[elevation_array == np.pi / 2.0]
    assert np.allclose(deltas[elevation_array == -np.pi / 2.0], 0), deltas[elevation_array == np.pi / 2.0]
    assert np.allclose(deltas[elevation_array == 0], np.max(deltas)), deltas[elevation_array == 0]
    assert np.allclose(deltas[elevation_array == np.pi], np.min(deltas)), deltas[elevation_array == np.pi]
