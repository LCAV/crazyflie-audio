...
def get_tetrahedron_array(dimension=2, baseline=0.05):
    ...
    tetrahedron = np.array([[baseline * np.sqrt(3) / 3, 0, 0], [-baseline * np.sqrt(3) / 6, baseline / 2., 0],
                            [-baseline * np.sqrt(3) / 6, -baseline / 2., 0], [0, 0, baseline * np.sqrt(6) / 3]])
...
def ambiguity_test(mic_positions, freq):
    ...
    print(f'signal wavelength/2: {lamda/2:.1e}, minimum distance between mics {delta:.1e}')
    print(
...
if __name__ == "__main__":
    ...
    for dimension in [2, 3]:
        mic_positions = get_uniform_array(num_mics, baseline=baseline, dimension=dimension)
        min_dist = get_min_distance(mic_positions)
        min_dist_theo = baseline / (num_mics - 1)
        assert abs(min_dist - min_dist_theo) < eps, f'{dimension}D test failed for uniform: {min_dist}!={min_dist_theo}'

        mic_positions = get_tetrahedron_array(baseline=baseline, dimension=dimension)
        min_dist = get_min_distance(mic_positions)
        assert abs(min_dist - baseline) < eps, f'{dimension}D test failed for tetra: {min_dist}!={baseline}'
        max_dist = get_max_distance(mic_positions)
        assert abs(max_dist - baseline) < eps, f'{dimension}D test failed for tetra: {max_dist}!={baseline}'
    print('all tests ok.')
