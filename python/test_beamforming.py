...
def prepare(mic_positions, gt_azimuth=30 * np.pi / 180):
    ...
    assert np.all([f in frequencies for f in frequencies_signal
    ...
    for freq in frequencies_signal:
        new_signals = np.array([np.cos(2 * np.pi * freq * (times - delay) + offset) for delay in delays])
    ...
    assert all([freq_bins[i] == np.where(f == frequencies)[0][0] for i, f in enumerate(frequencies_signal)])
...
def test_beamforming():
    ...
    for mic_name, mic_positions in mic_positions_dict.items():
        ...
        for deg in test_angles_deg:
            ...
            signals_f, Rx, frequencies_signal, freq_bins = prepare(mic_positions, gt_azimuth)
            ...
            lamda = 1e-3
            lcmv_zero = gt_azimuth + (30 * np.pi / 180.)

            for i, az in enumerate(azimuth_scan):
                constraints = [(az, 1)]
                h_mvdr_vector, *_ = get_lcmv_beamformer(Rx, frequencies_signal, mic_positions, constraints,
                                                        lamda=lamda)  # n_freqs x n_mics
                delays = get_mic_delays(mic_positions, azimuth=az)

                if az != lcmv_zero:
                    constraints = [(az, 1), (lcmv_zero, 0)]
                    h_lcmv_vector, *_ = get_lcmv_beamformer(Rx, frequencies_signal, mic_positions,
                                                            constraints)  # n_freqs x n_mics
                else:
                    constraints = [(az, 1)]
                    h_lcmv_vector, *_ = get_lcmv_beamformer(Rx, frequencies_signal, mic_positions,
                                                            constraints)  # n_freqs x n_mics

                h_das_vector = get_das_beamformer(az, frequencies_signal, mic_positions)  # n_freqs x n_mics
                ...
                for j, f in enumerate(frequencies_signal):
                    ...
                    np.testing.assert_allclose(power_y_das, power_y_das_test, atol=1e-10)

                    power_y_das_test2 = np.abs(h_das.conj().dot(signals_f[:, freq_bin]))**2 / n_mics
                    np.testing.assert_allclose(power_y_das, power_y_das_test2, atol=1e-10)

                    # check that phase of h exactly compensates for delays in ground truth direction.
                    if az == gt_azimuth:
                        np.testing.assert_allclose(
                            np.unwrap(np.angle(h_das)) / (2 * np.pi * f), -delays.reshape(h_das.shape))
                    ...
                    np.testing.assert_allclose(power_y_mvdr, power_y_mvdr_test, atol=1e-10)

                    # TODO is it normal that this atol has to be relatively high?
                    power_y_mvdr_test2 = np.abs(h_mvdr.conj().dot(signals_f[:, freq_bin]))**2 / n_mics
                    np.testing.assert_allclose(power_y_mvdr, power_y_mvdr_test2, atol=1e-5)
                    ...
                    power_y_mvdr_test3 = 1 / np.abs(a_vector.conj().dot(Rx_inv).dot(a_vector))
            ...
            for responses, name in zip(
                [responses_das, responses_mvdr],  # responses_lcmv],
                ['das', 'mvdr']):  #, 'lcmv']):
                combination_sum = np.sum(responses, axis=1)
                combination_product = np.product(responses, axis=1)
                ...
                assert abs(guess_sum - gt_azimuth) < angle_res / 2.0, f"failed for {name}, {deg}"
                assert abs(guess_product - gt_azimuth) < angle_res / 2.0, f"failed for {name}, {deg}"
