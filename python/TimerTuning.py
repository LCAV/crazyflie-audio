...
TARGET_Fs = [3000, 3125, 3250, 3375, 3500, 3625, 3750, 3875, 4000, 4125, 4250, 4375, 4500, 4625, 4750, 4875]
#TARGET_Fs = { 9391, 9672, 9953, 10266, 10594, 10938, 11328, 11734, 12172, 12641, 13141, 13688, 14281, 14922, 15641}
TARGET_F = 1188  # In Hz so 50.0 is 0.020 seconds period and 0.25 is 4 seconds period
CLOCK_MCU = 84000000
TOLERANCE = 0.00018
...
def close_divisor(psc, tolerance):
    ...
    if error < tolerance and arr < 65536.0 and arr > 2 and (int(arr) % 2) != 0:
...
for TARGET_F in TARGET_Fs:
	...
	freal = df.iloc[[0]]['F'].values[0]
	psc = df.iloc[[0]]['PSC'].values[0]
	arr = df.iloc[[0]]['ARR'].values[0]
	err = float(df.iloc[[0]]['ERROR'].values[0])*1000000


	#print(f'f = {TARGET_F} PSC = {psc}, ARR = {arr}, err = {err:.0f}')
	print(f'{{{TARGET_F}, {psc}, {arr}, {err:.0f}}},')
	#print(output[4:])
	#print(output[1:200])
#	print()
exit(0)